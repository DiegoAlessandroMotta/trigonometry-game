...
export class StartScreenScene extends Phaser.Scene {
  ...
  private createTitle() {
    ...
    this.titleText = this.add.text(384, 80, 'TRIGONOMETRÍA', {
      fontSize: '48px',
      color: '#00ffff',
      fontStyle: 'bold',
      stroke: '#000000',
    ...
    // Subtítulo
    this.subtitleText = this.add.text(384, 130, 'Juegos Educativos', {
      fontSize: '24px',
      color: '#ffffff',
      fontStyle: 'bold',
      stroke: '#000000',
  ...
  private createMenuButtons() {
    const buttonConfigs = [
      { text: 'JUGAR', action: () => this.scene.start('MainMenuScene'), color: 0xff4444, icon: '🎮' },
      { text: 'MANUAL', action: () => this.showManual(), color: 0x44ff44, icon: '📖' },
      { text: 'CRÉDITOS', action: () => this.showCredits(), color: 0x4444ff, icon: '👥' },
      { text: 'PRUEBA', action: () => this.showTest(), color: 0xffaa44, icon: '🧪' },
      { text: 'SALIR', action: () => this.exitGame(), color: 0xff44ff, icon: '🚪' }
    ]
    ...
    buttonConfigs.forEach((config, index) => {
      ...
      // Icono
      const iconText = this.add.text(-120, 0, config.icon, {
        fontSize: '20px'
      }).setOrigin(0.5)
  ...
  private showManual() {
    ...
    const title = this.add.text(384, 100, 'MANUAL DE JUEGO', {
      fontSize: '32px',
      color: '#00ffff',
      fontStyle: 'bold',
      stroke: '#000000',
    ...
    const content = this.add.text(384, 200, 
      '🎮 CÍRCULO UNITARIO\n' +
  ...
  private showCredits() {
    ...
    const title = this.add.text(384, 100, 'CRÉDITOS', {
      fontSize: '32px',
      color: '#00ffff',
      fontStyle: 'bold',
      stroke: '#000000',
  ...
  private showTest() {
    ...
    const title = this.add.text(384, 100, 'MODO PRUEBA', {
      fontSize: '32px',
      color: '#ffaa44',
      fontStyle: 'bold',
      stroke: '#000000',
    ...
    const playButton = new Button(
      this,
      300, 380,
      { text: 'Jugar Prueba', width: 120, height: 35 },
  ...
  private exitGame() {
    ...
    const title = this.add.text(384, 150, '¿SALIR DEL JUEGO?', {
      fontSize: '32px',
      color: '#ff4444',
      fontStyle: 'bold',
      stroke: '#000000',
    ...
    const message = this.add.text(384, 200, '¿Estás seguro de que quieres salir?', {
      fontSize: '18px',
      color: '#ffffff',
      align: 'center'
    }).setOrigin(0.5)
